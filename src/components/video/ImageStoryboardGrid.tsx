...
import { Plus, Loader2, Trash2, Image, Download } from 'lucide-react';
import { StoryboardImage } from '@/types/video';

interface ImageStoryboardGridProps {
...
export const ImageStoryboardGrid: React.FC<ImageStoryboardGridProps> = ({
    storyboardImages,
    selectedId,
    onSelect,
    onAdd,
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    {/* 批量下载按钮 */}
                    {onDownloadAll && storyboardImages.some((sb) => {
                        const activeVariant = sb.imageVariants?.find(v => v.id === sb.activeImageVariantId);
                        return (activeVariant?.status === 'completed' && activeVariant?.imageUrl) ||
                            (sb.status === 'completed' && sb.imageUrl);
                    }) && (
                            <button
                    ...
                    {/* 清空按钮 */}
                    {onClearAll && storyboardImages.length > 0 && (
                        <button
                            onClick={onClearAll}
            ...
            >
                <div className="flex gap-2 h-full">
                    {storyboardImages.map((storyboardImage, index) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => onSelect(storyboardImage.id)}
                                onMouseEnter={() => setHoveredId(storyboardImage.id)}
                                ...
                                >
                                    {currentImageUrl || currentThumbnailUrl ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center">
                                            <Image size={14} style={{ color: '#374151' }} />
                                        </div>
                                    )}
