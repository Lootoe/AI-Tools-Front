        ...
        {prop.thumbnailUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            {isGenerating ? (
              ...
              <Box size={24} style={{ color: 'rgba(107,114,128,0.4)' }} />
            )}
          </div>
        )}

        {/* 生成中遮罩 */}
        {isGenerating && prop.thumbnailUrl && (
...
// 新建物品卡片
const NewPropCard: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <div
    onClick={onClick}
...
export const PropsWorkspace: React.FC<PropsWorkspaceProps> = ({ scriptId }) => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; propId: string | null; propName: string }>({
    isOpen: false,
    propId: null,
    propName: '',
  });
  ...
  const hasDesignImage = !!selectedProp?.designImageUrl;

  const handleCreateProp = async () => {
    try {
      const propId = await addProp(scriptId, '新物品', '');
  ...
  const handleGenerateDesign = async () => {
    ...
    try {
      // 调用后端物品设计稿生成接口，提示词模板在后端
      const response = await generatePropDesign(editDescription.trim(), selectedModel);

      if (response.success && response.images.length > 0) {
        await updateProp(scriptId, selectedProp.id, {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !selectedProp || isProcessing) return;
  ...
  const handleDeleteClick = (prop: Prop) => {
    setDeleteConfirm({
      isOpen: true,
      propId: prop.id,
      propName: prop.name || '未命名物品',
    });
  };

  const handleConfirmDelete = async () => {
    if (deleteConfirm.propId) {
      try {
        await deleteProp(scriptId, deleteConfirm.propId);
        if (selectedPropId === deleteConfirm.propId) {
    ...
    setDeleteConfirm({ isOpen: false, propId: null, propName: '' });
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="删除物品"
        message={`确定要删除物品「${deleteConfirm.propName}」吗？此操作无法撤销。`}
        type="danger"
        confirmText="删除"
        cancelText="取消"
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              >
                <Box size={12} style={{ color: '#00f5ff' }} />
              </div>
              <span className="text-xs font-medium text-white">物品池</span>
            </div>
            <span className="text-[10px]" style={{ color: '#6b7280' }}>
              {props.length}
            </span>
          ...
          <div className="flex-1 overflow-y-auto p-2 scrollbar-thin">
            {isLoading ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                <NewPropCard onClick={handleCreateProp} />
                {props.map((prop) => (
                  <PropCard
                    key={prop.id}
                    prop={prop}
          ...
          {selectedProp ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-white">物品编辑器</span>
              </div>
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <Textarea
                      ...
                      placeholder="描述物品的材质、色调、细节、关联角色/场景等信息..."
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-white">物品设计稿</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {/* 模型选择下拉框 */}
                      ...
                      {hasDesignImage && !isProcessing ? (
                        <a
                          href={selectedProp.designImageUrl}
                          download={`prop-${selectedProp.name}-${Date.now()}.png`}
                  ...
                  <div className="flex-1 flex items-center justify-center p-4 overflow-auto relative">
                    ...
                    {selectedProp.designImageUrl ? (
                      <img
                        src={selectedProp.designImageUrl}
                        alt="物品设计稿"
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <div className="relative mb-6">
                ...
                >
                  <Box size={36} style={{ color: 'rgba(0,245,255,0.5)' }} />
                </div>
                <div
              ...
              <h3 className="text-lg font-semibold text-white mb-2">选择或创建物品</h3>
              <p className="text-sm max-w-md mb-4" style={{ color: '#6b7280' }}>
                从左侧物品池选择一个物品进行编辑，或点击"新建"创建新物品
              </p>
              <Button
