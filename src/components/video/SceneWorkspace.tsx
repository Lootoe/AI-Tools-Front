        ...
        {scene.thumbnailUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            {isGenerating ? (
              ...
              <Mountain size={24} style={{ color: 'rgba(107,114,128,0.4)' }} />
            )}
          </div>
        )}

        {/* 生成中遮罩 */}
        {isGenerating && scene.thumbnailUrl && (
...
// 新建场景卡片
const NewSceneCard: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <div
    onClick={onClick}
...
export const SceneWorkspace: React.FC<SceneWorkspaceProps> = ({ scriptId }) => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; sceneId: string | null; sceneName: string }>({
    isOpen: false,
    sceneId: null,
    sceneName: '',
  });
  ...
  const hasDesignImage = !!selectedScene?.designImageUrl;

  const handleCreateScene = async () => {
    try {
      const sceneId = await addScene(scriptId, '新场景', '');
  ...
  const handleGenerateDesign = async () => {
    ...
    try {
      // 调用后端场景设计稿生成接口，提示词模板在后端
      const response = await generateSceneDesign(editDescription.trim(), selectedModel);

      if (response.success && response.images.length > 0) {
        await updateScene(scriptId, selectedScene.id, {
  ...
  const handleSave = async () => {
    if (!selectedScene || isSaving) return;
    setIsSaving(true);
    try {
      await updateScene(scriptId, selectedScene.id, {
        name: editName.trim() || '未命名场景',
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !selectedScene || isProcessing) return;
  ...
  const handleConfirmDelete = async () => {
    if (deleteConfirm.sceneId) {
      try {
        await deleteScene(scriptId, deleteConfirm.sceneId);
        if (selectedSceneId === deleteConfirm.sceneId) {
    ...
    setDeleteConfirm({ isOpen: false, sceneId: null, sceneName: '' });
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="删除场景"
        message={`确定要删除场景「${deleteConfirm.sceneName}」吗？此操作无法撤销。`}
        type="danger"
        confirmText="删除"
        cancelText="取消"
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              >
                <Mountain size={12} style={{ color: '#00f5ff' }} />
              </div>
              <span className="text-xs font-medium text-white">场景池</span>
            </div>
            <span className="text-[10px]" style={{ color: '#6b7280' }}>
              {scenes.length}
            </span>
          ...
          <div className="flex-1 overflow-y-auto p-2 scrollbar-thin">
            {isLoading ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                <NewSceneCard onClick={handleCreateScene} />
                {scenes.map((scene) => (
                  <SceneCard
                    key={scene.id}
                    scene={scene}
          ...
          {selectedScene ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-white">场景编辑器</span>
              </div>
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <Textarea
                      ...
                      placeholder="描述场景的类型、氛围、色调、细节元素等信息..."
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-white">场景设计稿</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {/* 模型选择下拉框 */}
                      ...
                      {hasDesignImage && !isProcessing ? (
                        <a
                          href={selectedScene.designImageUrl}
                          download={`scene-${selectedScene.name}-${Date.now()}.png`}
                  ...
                  <div className="flex-1 flex items-center justify-center p-4 overflow-auto relative">
                    ...
                    {selectedScene.designImageUrl ? (
                      <img
                        src={selectedScene.designImageUrl}
                        alt="场景设计稿"
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">选择或创建场景</h3>
              <p className="text-sm max-w-md mb-4" style={{ color: '#6b7280' }}>
                从左侧场景池选择一个场景进行编辑，或点击"新建"创建新场景
              </p>
              <Button
