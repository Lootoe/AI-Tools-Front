...
const VariantCard: React.FC<VariantCardProps> = ({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isActive ? 'rgba(139,92,246,0.15)' : '#12121a',
        border: isActive ? '1px solid rgba(139,92,246,0.5)' : '1px solid #1e1e2e',
      }}
      onClick={onSelect}
    >
      <div className="flex items-center gap-2 p-2">
        {/* 缩略图 */}
        <div
          className="w-16 h-10 rounded flex-shrink-0 flex items-center justify-center overflow-hidden"
          style={{
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            {isGenerating && (
              <span className="text-[10px]" style={{ color: '#8b5cf6' }}>
                {variant.status === 'queued' ? '排队中' : `${variant.progress || 0}%`}
              </span>
            )}
            {isCompleted && (
          ...
          {variant.taskId && (
            <div
              className="text-[9px] mt-0.5 truncate max-w-[140px]"
              style={{ color: '#4b5563' }}
              title={variant.taskId}
            >
              ID: {variant.taskId}
            </div>
          )}
